...
export default function BudgetApp({ session }) {
  ...
  const chartData = useMemo(() => {
    ...
    return months.map((m) => {
      bal += netForMonth(m);
      return { month: m, label: monthLabel(m, true), solde: Math.round(bal) };
    });
  ...
  return (
    <div className="w-full min-h-screen bg-stone-50 font-sans text-stone-900">
      <div className="max-w-3xl mx-auto p-5 sm:p-8 space-y-6">
        <div className="flex items-start justify-between gap-4 flex-wrap">
          ...
          <div className="flex items-center gap-3">
            ...
            <button onClick={logout} className="p-2 rounded-md border border-stone-300 hover:bg-stone-100 text-stone-500" aria-label="Se déconnecter">
              <LogOut size={16} />
            </button>
          </div>
        </div>
        ...
        {errorMsg && (
          <div className="text-sm text-rose-700 bg-rose-50 border border-rose-200 rounded-md px-3 py-2">{errorMsg}</div>
        )}
      ...
      <div className="flex justify-end gap-2 pt-1">
        ...
        <button
          onClick={() => {
            ...
            onSubmit({ label: label.trim(), categoryId, amount: amt, month, recurringEnd: recurring ? recurringEnd : null });
          }}
